...
//    Renumbered selectable Board Locations to be from 1-9 instead of 0-8 to
...
*/
    ...
    {
        string_int.clear();
        string_int=to_string(i+1);
        strcpy(&Board[i],string_int.c_str());
    }
    // Show the board
...
{
 int i,j;
 int winning_row=0,winning_col=0,winning_diag=0;
...
//
 for(i=0 ; i<3; i++)
 {
    ...
    different_values=0;
    winner=Board[i];
    for(j=i+3 ; j<7+i ; j=j+3)
    {
        if(Board[j-3] != Board[j]) different_values++;
    }
    if(different_values == 0)
    {
...
//
    ...
    for(j=4 ; j<7 ; j=j+2)
    {
        if(Board[j-2] != Board[j]) different_values++;
    }
    if(different_values == 0)
    {
...
//
    ...
    for(j=4 ; j<9 ; j=j+4)
    {
        if(Board[j-4] != Board[j]) different_values++;
    }
    if(different_values == 0)
    {
